...
### 手動 Web Serial DisConnect（[#753](https://github.com/gurezo/chirimen-lite-console/issues/753)）

ヘッダーメニューの **Web Serial DisConnect** も、ログアウト完了時と同じリセット手順を使う（`ConsoleShellComponent.endSessionAndReturnToConnect`）。

```text
接続済み UI
  -> ユーザーが Web Serial DisConnect を選択
  -> notifyManualDisconnect()（トースト）
  -> closeDialog / dialogService.closeAll()
  -> connectionVm.disconnect()
  -> （以降はログアウト完了時と同じ disconnect$ → 未接続 UI）
```

要点:

- **ダイアログを閉じてから切断する**。切断完了まで操作可能なままダイアログが残らないようにする。
- **二重切断を防ぐ**。切断処理中・`logoutPending`・`connectionBusy`・未接続時は `onDisConnect` を no-op とする。
- **通知文言のみ区別する**。リセット手順は共通で、トーストは `notifyManualDisconnect()`（手動切断）と `notifyLogoutDetected()`（ログアウト検出）を使い分ける。
...
- [#753](https://github.com/gurezo/chirimen-lite-console/issues/753) — 手動 Web Serial DisConnect をログアウトと同じリセット挙動にする（本ドキュメント同節の「手動 Web Serial DisConnect」）
