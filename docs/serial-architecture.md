...
```bash
npx nx run libs-web-serial-data-access:test
npx nx run libs-web-serial-util:test
npx nx run libs-web-serial-data-access:lint
npx nx run libs-web-serial-util:lint
npx nx run apps-console:test
```

`libs-web-serial-data-access` のテストには `SerialCommandPipelineService`（並列 `exec` の直列化・`readUntilPrompt$` 等）および Pi Zero ブートストラップ周りの spec が含まれる。ブラウザ実機（`terminalText$` の `\r` 表示、二重表示の有無、Pi Zero ログイン〜setup）は上記コマンドでは代替できないため、**マージ前に手元で接続確認**し、PR に環境と結果を記録する。
