...
| 用途 | 使用する stream |
| --- | --- |
| ターミナル表示（terminal helper で整形済みテキスト） | `terminalText$`（= `createTerminalBuffer(session.receive$).text$`） |
...
- **`terminalText$`（facade）** = `createTerminalBuffer(session.receive$).text$`。xterm など **UI 表示専用**。プロンプト検出に使わない。
