...
| `exec$` / `readUntilPrompt$`・キュー | `serial-command-pipeline.service.spec.ts`（`describe: SerialCommandPipelineService` / `SerialCommandPipelineService (queue)`） |
| 汎用 `prompt` / `RegExp` 照合（`matchesSerialPrompt`） | `serial-prompt-match.spec.ts` |
